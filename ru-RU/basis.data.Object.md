...
При сбросе делегата объект создает копию свойств `data` и `state` и перестает разделять их с другими. При этом события `update` и `stateChanged` не выбрасывается (в сущности для экземпляра ничего не меняется).
