...
Если с данными объекта происходят изменения, то так же выбрасывается событие `update`. Не зависимо от количества изменных ключей, событие всегда одно и оно так же содержит дельту изменений (такую же как вызова метода `update`). Поэтому не сложно следить за изменениями в объекте:
...
Как можно заметить, ключи и их значения никак не ограничиваются – они могут быть любыми. Это сделано намеряно, чтобы экземпляры этого класса несли основной функционал и были максимально дешевыми (время и память). [xxx] - ссылка на слайд сравнения.
...
Объекты поддерживают механизм делегирования, который позволяет нескольким объектам ссылаться на одни и те же данные. Делается это путем связывания объектов, мы говорим, что объект делегирует данные другого объекта. В этом случае объект хранит ссылку на объект, чьи данные он делегирует, в свойстве `delegate`. Меняется делегат методом `setDelegate`.
...
Так как оба объекта ссылаются на одни и те же данные, то становится не важным у кого из них вызывать метод `update`, чтобы изменить данные. Если вызов метода `update` приводит к изменениям, событие `update` будет выброшено для обоих объектов.
...
Понятие состояния является очень важным, определяет состояние данных и что с ними происходит. Значение состояния хранится в поле `state`
