...
И подправим немного `heroes-details`. Во второй главе данные об `id` и `title` хранились в объекте `Hero`. Сейчас же у нас все данные лежать плоско, поэтому изменим код обновления биндингов и удалим жестко заданные данные в `data`:
