...
В этой главе мы добавим возможность редактировать героев в списке, а так же разберем важное понятие сателлитов в `basis.js`.
...
Для того чтобы использовать компонент в качестве сателлита необходимо в ноде указать свойство `satellite` и в качестве ключа указать имя сателлита по-вашему усмотрения, а в качестве значения свойства должен быть ваш компонент.
...
Все `basis.ui.node` умеют свойство `selected`, которое по-умолчанию равно `false`.
...
Т.к. мы уже знаем что осуществить передачи данных между компонентами можно при помощи делегатов нам осталось только передать выбранный элемент в качестве делегата нашему компоненту `hero-details`.
