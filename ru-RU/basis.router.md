...
Модуль `basis.router` позволяет отслеживать изменения `location` и завать реакцию на эти изменения.
...
По умолчанию состояние `location.hash` не проверяется и изменения не отслеживаются. Для того модуль начал отслеживать изменения необходимо вызвать функцию `basis.router.start()`. Последующие вызовы этой функции не имеют эффекта, если изменения уже отслеживаются. Чтобы модуль прекратил отслеживать изменения, используется функция `basis.router.stop()`. Когда начинаются отслеживаться изменения, срабатывают уже добавленные "реакции", которые удовлетворяют текущему значению `location.hash`.

## Реакция

Реакция – это набор функций, которые задаются для определенного значения `location.hash`, и выполняемые при определенных условиях (событиях).

Чтобы добавить "реакцию" для некоторого значения `location.hash`, используется функция `basis.router.add`, а чтобы удалить – функция `basis.router.remove`. При этом функции `basis.router.remove` должны передаваться те же значения, что и для функции `basis.router.add`. Добавлять и удалять "реакции" можно в любой момент, не зависимо от того отслеживаются ли изменения `location.hash` или нет. При этом, если отслеживаются изменения и добавляемая "реакция" удовлетворяет условиям, то она тут же срабатывает.
...
- `callbacks` – набор функций, выполняемые при наступлении определенных событий;
...
Слеши в конце пути считаются частью пути, поэтому `'docs'` и `'docs/'` будут считать разными путями. Чтобы указать реакцию для обоих случаем необходимо использовать скобки, например `'docs(/)'`.

В простых случаях описанных правил оказывается досточно. Но в сложных случаях или когда возможностей оказывается недостаточно, оказывается проще описать непосредственно регулярное выражение. В этом 
...
- для "реакций", которые матчились при предыдущей проверке, но перестали матчиться выполняются их `leave`-функция;
- для "реакций", которые не матчились при предыдущей проверке, но начали матчится при текущей, выполняются их `enter`-функция;
- для всех реакций, которые удовлетворяют текущему значению `location.hash`, выполняется их `match`-функция.
...
Свойство `debug` с заданным значением `true` позволяет получать отладочную информацию: обнаружение факта изменения `location.hash` и какие реакции были применены.
