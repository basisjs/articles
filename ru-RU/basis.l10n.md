...
У токенов есть тип, который задается в секции `_meta` описания словаря, в секции `type`. Тип токена является одинаковым для всех культур словаря.
...
* `markup` - строка, которая которая может содержать разметку.
