...
- `pipe()` – преобразование `Emitter`'а хранимого в качестве значения
- `query()` – подзапрос к значению

### Value#as(fn)

Метод `as` возвращает экземпляр `ReadOnlyValue`, который хранит преобразованное значение. Метод принимает единственный параметр – преобразующую функцию.
