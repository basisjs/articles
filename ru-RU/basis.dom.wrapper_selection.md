...
- `unselect(multiple)` – устанавливает значение `false`;
- `setSelected(selected, multiple)` - устанавливает значение в зависимости от переданного значения).
...
> Стоит помнить, что значение `selected` должно меняться исключительно указанными выше методами. Для чего используется параметр `multiple` будеь описано ниже.

При установке свойства `selected` в значение `true`, выбрасывается событие `select`, при установке в значение `false` – `unselect`. Начальное значение `selected` можно задать при создании узла, но событие `select` при этом выбрасываться не будет.
...
Сам контекст выделения представляет собой набор, экземпляр специального класса `basis.dom.wrapper.Selection`. Этот класс унаследован от `basis.data.Dataset` и имеет тот же интерфейс. Его главной особенностью является синхронизация состава со значением свойства `selected` его членов. Так, если добавить в такой набор узел, то его свойство `selected` будет выставлено в `true`, а если удалить из набора, то у удаленного узла `selected` станет равно `false`. Справедливо и обратное, если узлу выставить значение `selected` в `true`, то он будет автоматически добавлен в набор, указанный в `contextSelection`, а если в `false`, то удален из него.
...
Не смотря на то, что `contextSelection` можно выставить при создании узла, его не предполагается задавать явно. Обычно контекст выделения узлы заимствуют у родителя, при назначении нового родителя.
...
Когда узел отвязывается от своего родителя, то значение `contextSelection` выставляется в `null`, при условии, что `contextSelection` равен `selection` родителя или, значение не задано (равно `null`), равно `contextSelection` родителя. То есть ситуация обратная привязыванию родителю.
...
В качестве `selection` можно задать только экземпляр `basis.dom.wrapper.Selection`. Если задается другое значение иное значение, которое приводится к `true`, то это значение считается конфигом и экземпляр `Selection` создается неявно. По этой причине отпадает необходимость импортировать класс `Selection` из модуля `basis.dom.wrapper`.
...
Как видно из примера, выделить несколько узлов не сложно, имея доступ к узлу (в данном случае `list`) и его `selection`. Но обычно удобнее управлять выделенностью узла в рамках логики самого узла, то есть пользоваться его методами `select()` и `setSelected(true)`. Однако, если вызывать последовательно для ряда узлов, выбранным окажется только последний. Это происходит по причине того, что эти методы по умолчанию используют режим одиночного выделения.
...
Чтобы использовать режим множественного выбора, для методов `select` и `setSelected` необходимо передавать дополнительный параметр `multiple` равный `true`. Однако такой режим не подразумевает, что узел будет только добавляться к выбранным. В таком режиме значение `selected` меняется как переключатель (`toggle`), то если узел еще не выбран, то он становится выбранным, иначе выделение снимается. При этом значение `selected` других узлов в контексте остается неизменным. Поэтому без дополнительных проверок получается инверсия выделения.
...
Чтобы гарантированно выделить все необходимые узлы, нужно использовать такую проверку.
...
Может показаться странной логика работы множественного выделения. Но все встает на свои места, если учесть, что механизм ориентирован на взаимодействие с пользователем. Так режим множественного выделения используется, если зажата клавиша `ctrl` (Windows) или `meta` (Mac). С описанным выше подходом задача решается так:
...
Начиная с версии `1.4` в качестве значения `selected` можно задать объектом с [`binding bridge`](bindingbridge.md) интерфейсом (`bb-value`). В этом случае `selected` будет автоматически синхрозироваться с таким объектом (его значение приводится к `boolean`). При этом `selected` по прежнему хранит `true` или `false`, и при его изменении срабатывают события `select` и `unselect`. Связь с `bb-value` "прячется" в приватном свойстве `selectedRA_`, в котором хранится специальный адаптер-наблюдатель.
...
- для привязки `bb-value` необходимо использовать метод `setSelected(newValue)`.
...
- [использование набора выбранных узлов как источных данных](//basisjs.com/basisjs/demo/common/selection_datasource.html) для других компонент.
