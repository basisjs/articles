...
Второстепенные/вспомогательные элементы могут маркироваться атрибутом `b:role` со значением. Например, если в описании компонента указана роль foo, а в шаблоне прописан атрибут `b:role="bar"`, то в итоговой разметке у элемента будет роль `foo/bar`.
...
В самом демо можно можно открыть консоль в панели разработчика, покликать по элементам списка и увидеть отчеты трекинга.
