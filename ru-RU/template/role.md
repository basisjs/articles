...
В шаблоне главный элемент компонента маркируется атрибутом `b:role`. Значение роли всегда приходит из описании компонента (владельца шаблона), это значение специального биндинга.
