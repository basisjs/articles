...
Метод `getValues()` позволяет получить список значений, полученных от каждого элемента набора. Методу в качестве параметра передается функция-геттер или строка, из которой генерируется такая функция (используется `basis.getter()`).
...
Этот класс предоставляет следующие методы:
  ...
  * `set(items)` - задание состава элементов;
...
Синхронизация работает следующим образом. Пусть текущий список элементов это `A`, а переданный список – `B`. Если элемент присутствует в обоих списках, то никаких действий не производится. Если элемент есть только в `A`, но отсутствует в `B`, то элемент разрушается (вызывается его метод `destroy`). Если элемента нет в списке `A`, но он есть в `B`, то он добавляется в набор.
