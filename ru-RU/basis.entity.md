...
- [Создание типа](#создание-типа)
  ...
  - [Описание полей](#описание-полей)
    ...
    - [Псевдонимы полей](#псевдонимы-полей)
  ...
  - [Накопление и откат изменений \(rollback\)](#накопление-и-откат-изменений-rollback)
  - [Type.reader](#typereader)
  - [Type.all](#typeall)
  - [Расширение класса типа](#расширение-класса-типа)
- [EntitySet](#entityset)
- [Построение инфраструктуры](#построение-инфраструктуры)
...
- `type` – обычно это функция нормализации значения, которая приводит новое значение к необходимому виду (типу) (см. типы полей); так же в качестве типа могут быть заданы массив или строка, которые заменяются на функции при создании типа;
...
Так же в качестве описания поля могут выступать специальные функции модуля: `basis.entity.IntId`, `basis.entity.NumberId` и `basis.entity.StringId`. Эти функции так же становятся "типом", но дополнительно определяют что поле является первичным ключом (или его частью).

#### Значение по умолчанию

Значение по умолчанию задается настройкой `defValue` (сокращение от `default value`) в описании поля. Это может быть любое значение или функция. Такие значения используется только в том случае, если в данных переданных конструктору экземпляра типа не указано соответствующее поле (`field in data == false`).
